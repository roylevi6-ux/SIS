...
class TestListTranscripts:
    ...
    def test_list_transcripts_returns_200(self, mock_svc, client):
        ...
        resp = client.get("/api/transcripts/acct-1")
    ...
    @patch("sis.api.routes.transcripts.transcript_service")
    def test_list_transcripts_default_active_only(self, mock_svc, client):
        mock_svc.list_transcripts.return_value = []
        client.get("/api/transcripts/acct-1")
        mock_svc.list_transcripts.assert_called_once_with("acct-1", active_only=True)

    @patch("sis.api.routes.transcripts.transcript_service")
    def test_list_transcripts_all(self, mock_svc, client):
        mock_svc.list_transcripts.return_value = []
        client.get("/api/transcripts/acct-1?active_only=false")
        mock_svc.list_transcripts.assert_called_once_with("acct-1", active_only=False)

    @patch("sis.api.routes.transcripts.transcript_service")
    def test_list_transcripts_empty(self, mock_svc, client):
        mock_svc.list_transcripts.return_value = []
...
class TestUploadTranscript:
    ...
    @patch("sis.api.routes.transcripts.transcript_service")
    def test_upload_transcript_returns_response(self, mock_svc, client):
    ...
    @patch("sis.api.routes.transcripts.transcript_service")
    def test_upload_transcript_passes_correct_params(self, mock_svc, client):
        mock_svc.upload_transcript.return_value = _make_transcript_orm()
        participants = [{"name": "Rep", "role": "AE", "company": "Riskified"}]
    ...
    @patch("sis.api.routes.transcripts.transcript_service")
    def test_upload_transcript_minimal_body(self, mock_svc, client):
        mock_svc.upload_transcript.return_value = _make_transcript_orm()
        resp = client.post("/api/transcripts/", json={
            "account_id": "acct-1",
    ...
    def test_upload_transcript_missing_required_fields(self, client):
        resp = client.post("/api/transcripts/", json={"account_id": "acct-1"})
        assert resp.status_code == 422

    def test_upload_transcript_missing_account_id(self, client):
        resp = client.post("/api/transcripts/", json={
            "raw_text": "text",
            "call_date": "2025-07-01",
        })

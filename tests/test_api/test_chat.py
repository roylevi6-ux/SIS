...
class TestChatQuery:
    ...
    def test_query_returns_response(self, mock_svc, client, auth_headers):
    ...
    @patch("sis.api.routes.chat.query_service")
    def test_query_passes_message_and_history(self, mock_svc, client, auth_headers):
    ...
    @patch("sis.api.routes.chat.query_service")
    def test_query_no_history_passes_empty_list(self, mock_svc, client, auth_headers):
        mock_svc.query.return_value = "Answer"
        client.post("/api/chat/query", json={"message": "Question"}, headers=auth_headers)
        mock_svc.query.assert_called_once_with("Question", [])

    @patch("sis.api.routes.chat.query_service")
    def test_query_null_history_passes_empty_list(self, mock_svc, client, auth_headers):
        mock_svc.query.return_value = "Answer"
        client.post(
            "/api/chat/query",
    ...
    @patch("sis.api.routes.chat.query_service")
    def test_query_returns_no_data_message(self, mock_svc, client, auth_headers):
